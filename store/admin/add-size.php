...
if(isset($_REQUEST['PageType']) && isset($_REQUEST['ID']) && $_REQUEST['PageType']=='Edit' && $_REQUEST['ID']!='')
{
	...
	$Content = $Web->getRecord($_REQUEST['ID'], "TableID", "p_size");
	if($Content["TableID"]=='')
		$Web->Redirect("index.php?page=product-size");
}
?>
<div class="Page-Data">
<h1>Add New Size</h1>
<form name="RecordForm" id="RecordForm" method="post" action="" onsubmit="return ValidateSize();" enctype="multipart/form-data">
<input type="hidden" name="ValidateSizeFlag" id="ValidateSizeFlag" />

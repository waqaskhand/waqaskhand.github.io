...
if(isset($_REQUEST['PageType']) && isset($_REQUEST['ID']) && $_REQUEST['PageType']=='Edit' && $_REQUEST['ID']!='')
{
	...
	$Content = $Web->getRecord($_REQUEST['ID'], "TableID", "p_brands");
	if($Content["TableID"]=='')
		$Web->Redirect("index.php?page=product-brands");
...
<form name="RecordForm" id="RecordForm" method="post" action="" onsubmit="return ValidateBrand();" enctype="multipart/form-data">
<input type="hidden" name="ValidateBrandFlag" id="ValidateBrandFlag" />
